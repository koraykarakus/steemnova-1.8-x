...
for ($i = 0;$i <= count($get_bots_techs) - 1;$i++) {
	...
	$planet_resources = mysqli_fetch_all(mysqli_query($connection, "SELECT small_cargo, floor(metal), floor(crystal), shipyard, id, galaxy, system, planet FROM uni1_planets WHERE id_owner=$id_owner")) [0];
	...
	if ($planet_resources[0] == 0 && $get_bots_techs[$i][3] >= 2 && $planet_resources[1] >= 2000 && $planet_resources[2] >= 2000 && $planet_resources[3] >= 2) {
	    ...
	    mysqli_query($connection, "UPDATE uni1_planets SET small_cargo = small_cargo + 1 WHERE id=$id_planet");
	    $planet_resources[0]++;
	}
	if ($planet_resources[0] > 0) {
	    ...
	    mysqli_query($connection, "UPDATE uni1_planets SET small_cargo = small_cargo - 1 WHERE id=$fleet_start_id");
