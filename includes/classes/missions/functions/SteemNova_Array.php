...
{
    $attack = [['attack' => 0, 'shield' => 0]][0];
    $defense = [['attack' => 0, 'shield' => 0]][0];
    // attackers shoot
    ...
    return [['attack' => $attack['attack'], 'defense' => $defense['attack'], 'attackShield' => $defense['shield'], 'defShield' => $attack['shield']]][0];
}
        ...
        foreach ($attacker['unit'] as $element => $amount)
        {
            ...
            for ($ship = 0; $ship < $amount; $ship++, $iter++)
            {
                if ($firstInit)
                {
                    ...
                    $fleets[$fleetID]['units'][] = ['unit' => $element, 'shield' => $thisShield, 'armor' => $thisArmor, 'att' => $thisAtt];
                }
...
function calculateAttack(&$attackers, &$defenders, $FleetTF, $DefTF)
{
    ...
    for ($ROUNDC = 0; $ROUNDC <= MAX_ATTACK_ROUNDS; $ROUNDC++)
    {
        ...
        $ROUND[$ROUNDC] = ['attackers' => $attackers, 'defenders' => $defenders, 'attackA' => $att['attackAmount'], 'defenseA' => $def['attackAmount'], 'infoA' => $att['attArray'], 'infoD' => $def['attArray']];
