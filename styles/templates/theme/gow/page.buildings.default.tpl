...
{block name="content"}
  ...
  <div class="ItemsWrapper">
    ...
    <div style="background:url('{$dpath}images/buildings.webp');"
      class="itemShow d-flex justify-content-center align-items-center w-100 bg-black position-relative border-orange">

      {foreach $BuildInfoList as $ID => $Element}
        <div id="item_big_{$ID}"
          class="buildItemBig position-absolute top-0 left-0 d-flex flex-column d-none rounded border border-1 border-dark p-0 m-0 w-100">
          <div class="d-flex w-100 itemTop">
            <div class="d-flex align-items-start justify-content-center bg-black">
              <img class="mx-2 hover-pointer" onclick="return Dialog.info({$ID})" src="{$dpath}gebaeude/{$ID}.gif"
            ...
            <div class="d-flex flex-column w-100 bg-light-black">
              <div class="bg-blue d-flex justify-content-start mb-2 text-white fw-bold">
                <div class="d-flex px-2">
                  <span class="fs-12 {if $Element.costOverflowTotal > 0}color-red hover-pointer{else}color-yellow{/if} p-0"
                    {if $Element.costOverflowTotal > 0} data-bs-toggle="tooltip" data-bs-placement="left"
            ...
            <table class='table-tooltip fs-11'>
              ...
              <tbody>
                ...
                {/foreach}
                    ...
                    {if !empty($Element.infoEnergyLong)}
                      <span
                        class="d-flex fs-12 p-0 hover-pointer {if {$Element.infoEnergyShort} > 0}color-green{else}color-red{/if}"
                        data-bs-toggle="tooltip" data-bs-placement="left" data-bs-html="true"
                        title='<span class="fs-12">{$LNG.bd_next_level}&nbsp;{$Element.infoEnergyLong}</span>'>&nbsp;<i
                          class="bi bi-activity"></i>&nbsp;{$Element.infoEnergyShort}</span>
                    {/if}
                  </div>
                    ...
                    <span class="d-flex flex-column">
                      {foreach $Element.costResources as $RessID => $RessAmount}
                        <div class="d-flex align-items-center my-1">
                          ...
                          <span
                            class="mx-1 fs-11 {if $Element.costOverflow[$RessID] == 0}text-white{else}color-red{/if}">{$RessAmount|number}</span>
                        </div>
                      {/foreach}
                    </span>
                  </div>
                    ...
                    {if $Element.level > 0}
                      ...
                      {if ($ID == 44 && !$HaveMissiles) ||  $ID != 44}<br>
                        <form action='game.php?page=buildings' method='post' class='build_form'>
                          ...
                          <input type='hidden' name='building' value='{$ID}'>
                          <button type='submit' class='button-downgrade' data-bs-toggle="tooltip" data-bs-placement="left"
                            data-bs-html="true" title="<table class='table-tooltip fs-11'>
                                                <thead>
                                                  <tr>
              ...
              <div class="d-flex flex-column w-100 bg-light-black itemInfo overflow-auto scroll">
                <span
                  class="fs-10 px-2 my-1 text-white w-100 fw-bold">{$LNG.fgf_time}&nbsp;:&nbsp;{pretty_time($Element.elementTime)}</span>
        ...
        <div class="d-flex flex-wrap justify-content-start bg-black pb-2 border-orange">
          <div class="d-flex w-100 justify-content-start m-2">
            <span
              class="color-yellow fs-12 fw-bolt">{$LNG.lm_buildings}&nbsp;(&nbsp;{$usedField}&nbsp;/&nbsp;{$maxField}&nbsp;)</span>
            <span class="color-yellow fs-12 fw-bolt">&nbsp;|&nbsp;{$LNG.st_points}&nbsp;[{$userBuildPoints}]</span>
          </div>
          <div class="mx-2 d-flex flex-wrap">
            {foreach $BuildInfoList as $ID => $Element}
              <div class="buildItemSmall position-relative d-flex user-select-none" onclick="showItem({$ID})"
                id="item_small_{$ID}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" title="{$LNG.tech.{$ID}}
                  ...
                  {/if}">
                  <div
                    class="levelInfo d-flex align-items-center justify-content-center position-absolute bottom-0 end-0 text-yellow bg-dark fs-11">
                    {$Element.level}</div>
                ...
                <form action="game.php?page=buildings" method="post" class="position-absolute top-0 left-0">
                  <input type="hidden" name="cmd" value="insert">
                  <input type="hidden" name="building" value="{$ID}">
                  <button type="submit" class="button-upgrade-small position-absolute top-0 left-0 d-flex"
