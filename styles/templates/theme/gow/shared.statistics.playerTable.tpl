...
<tr>
	...
	<td>
		<a href="#" onclick="return Dialog.Playercard({$RangeInfo.id}, '{$RangeInfo.name}');"{if $RangeInfo.id == $CUser_id} style="color:lime"{/if}>{$RangeInfo.name}&nbsp;</a>{if $RangeInfo.is_leader}<a style="color:yellow"  data-bs-toggle="tooltip"
	data-bs-placement="left"
	...
	</table>"><i class="fas fa-trophy"></i></a>{/if} {if $RangeInfo.id != $CUser_id && !empty($RangeInfo.class)}{foreach $RangeInfo.class as $class}{if !$class@first}&nbsp;{/if}<span class="galaxy-short-{$class} galaxy-short">{$ShortStatus.$class}</span>{/foreach}{/if}</td>
