  ...
  {foreach $AllPlanets as $PlanetRow}
    ...
    <div class="left">
      <a class="type_planet" href="game.php?page={$page}&amp;cp={$PlanetRow.id}">
        ...
        {if !empty($PlanetRow)}
        <img class="{if $PlanetRow.selected}border-yellow{/if}" src="{$dpath}planeten/{$PlanetRow.image}.jpg" width="33" height="33" alt="{$PlanetRow.name}">
        {/if}
      </a>
      {if isset($PlanetRow.moonInfo)}
      <a class="type_moon" href="game.php?page={$page}&amp;cp={$PlanetRow.moonInfo[0].id}">
        ...
        <img class="{if $PlanetRow.moonInfo[0].selected}border-yellow{else}hover-border-yellow{/if}" src="{$dpath}planeten/{$PlanetRow.moonInfo[0].image}.jpg" width="20" height="20" alt="{$PlanetRow.moonInfo[0].name}">
      </a>
      {/if}
    </div>
    <div class="right">
