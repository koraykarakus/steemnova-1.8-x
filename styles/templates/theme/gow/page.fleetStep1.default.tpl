		...
		{if isModuleAvailable($smarty.const.MODULE_SHORTCUTS)}
			<table class="table table-gow table-sm fs-12">
				...
				{foreach $shortcutList as $shortcutID => $shortcutRow}
					{if ($shortcutRow@iteration % $themeSettings.SHORTCUT_ROWS_ON_FLEET1) === 1}<tr style="height:20px;"
						class="shortcut-row">{/if}
						...
						{if $shortcutRow@last && ($shortcutRow@iteration % $themeSettings.SHORTCUT_ROWS_ON_FLEET1) !== 0}
							...
							{/for}
						{/if}
						{if ($shortcutRow@iteration % $themeSettings.SHORTCUT_ROWS_ON_FLEET1) === 0}
					</tr>{/if}
				{foreachelse}
		...
		<table class="table table-gow table-sm table-striped fs-12" style="table-layout: fixed;">
			...
			{foreach $colonyList as $ColonyRow}
				{if ($ColonyRow@iteration % $themeSettings.COLONY_ROWS_ON_FLEET1) === 1}<tr style="height:20px;">{/if}
					...
					{/if}
					{if ($ColonyRow@iteration % $themeSettings.COLONY_ROWS_ON_FLEET1) === 0}
				</tr>{/if}
			{foreachelse}
				<tr style="height:20px;">
