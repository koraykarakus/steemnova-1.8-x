    ...
    function getResult() {
      $.post("game.php?page=flightSimulator&mode=calcFleetSpeed&ajax=1", $('#flightSimForm').serialize(), function(data) {
        ...
        $.each(data, function(index, seconds) {
          ...
          $('#result').append('<tr class="resultInfo"><td>' + index * constant + '</td><td>' + seconds +
            '</td><td>' + readableTime + '</td></tr>');
          i++;
        });
      });
    }
    ...
    <tbody>
      <form id="flightSimForm" action="" method="post">
        ...
        <tr>
          ...
          <td><input onkeyup="getResult();"
              class="coordinate " name="endGalaxy"
              value="1"></td>
          <td><input onkeyup="getResult();"
              class="coordinate " name="endSystem"
              value="1"></td>
          <td><input onkeyup="getResult();"
              class="coordinate " name="endPlanet"
              value="1"></td>
        </tr>
        <tr>
          <td class="text-center" colspan="4">
            <button class="button_blue" onclick="clearShips();getResult();" type="button" name="button">Clear
              Ships</button>
            <button class="button_blue" onclick="clearTech();getResult();" type="button" name="button">Clear Tech</button>
            <button class="button_blue" onclick="clearCoordinates();getResult();" type="button" name="button">Clear
              Coordinates</button>
          </td>
        </tr>
      </form>
