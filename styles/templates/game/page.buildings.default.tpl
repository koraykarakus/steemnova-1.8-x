...
{block name="content"}
  ...
  <div class="items_wrapper">
    <div class="top" style="background:url('{$dpath}images/buildings.jpg');" class="bg-black border-orange">
          ...
          <div class="top">
            ...
            <div class="right">
                  ...
                  {if $element.cost_overflow_total > 0}
                    <div class="tooltip tooltip_bottom">
                      <table>
                        ...
                        <tbody>
                          {foreach $element.cost_overflow as $ResType => $ResCount}
                            <tr>
              ...
              <div class="requirements">
                        ...
                        <form action='game.php?page=buildings' method='post' class='destroy_form'>
                          ...
                          <div class="tooltip tooltip_bottom">
                            <table>
                              ...
                              <tbody>
                                {foreach $element.destroy_resources as $ResType => $ResCount}
                                  <tr>
                                    <td>{$LNG.tech.{$ResType}}</td>
                                    <td>
                                      <span
                                        style='color:{if empty($element.destroy_overflow[$ResType])}lime{else}#ffd600{/if}'>{$ResCount|number}</span>
                                    </td>
                                  </tr>
                                {/foreach}
                                <tr>
                ...
                <div class="bottom">
                  <div class="left">
                    {foreach $element.cost_resources as $RessID => $RessAmount}
                      <div class="resource">
                        ...
                        <span
                          class="{if $element.cost_overflow[$RessID] == 0}{else}color-red{/if}">{$RessAmount|number}</span>
                      </div>
                    {/foreach}
                  </div>
                  <div class="right">
      ...
      {/foreach}
      <span class="page_title">{$current_pname} - {$LNG.lm_buildings}</span>
    ...
    <div class="bottom">
      <div class="title">
        ...
        <span class="color-yellow">&nbsp;|&nbsp;{$LNG.st_points}&nbsp;[{$user_build_points}]</span>
      </div>
      <div class="list">
        {foreach $build_info_list as $id => $element}
          <div class="item_small" onclick="showItem({$id})" id="item_small_{$id}">
                ...
                <tbody>
                  {if !$element.technology_satisfied && !empty($element.requirements)}
                    ...
                    {foreach $element.requirements as $currentRequire}
                      <tr>
                        <td class='color-red'>
                          <img class='mx-2 hover-pointer' src='{$dpath}elements/{$currentRequire.require_id}.gif'
                            alt='{$LNG.tech.{$currentRequire.require_id}}' width='30' height='30'>
                        </td>
                        <td class='color-red align-middle text-start'><span
                            class='color-blue'>{$LNG.tech.{$currentRequire.require_id}}</span>&nbsp;({$currentRequire.needed_level}&nbsp;/&nbsp;<span
                            class='color-yellow'>{$currentRequire.current_level}</span>)</td>
                      </tr>
                    {/foreach}
                  {/if}
                </tbody>
            ...
            {if !$can_build_element || !$room_is_ok || !$element.buyable || !$element.technology_satisfied || ($is_busy.research && ($id == 6 || $id == 31)) || ($is_busy.shipyard && ($id == 15 || $id == 21))}
