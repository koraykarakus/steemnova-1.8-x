...
{block name="content"}
	<table class="table_game table_full">
		...
		{if !empty($otherRequestList)}
			<tr>
				<th colspan="5" style="text-align:center">{$LNG.bu_requests}</th>
			</tr>
			<tr>
				<th>{$LNG.bu_player}</th>
			...
			{foreach $otherRequestList as $otherRequestID => $otherRequestRow}
				<tr>
					...
					<td>{if {$otherRequestRow.ally_name}}<a
							href="game.php?page=alliance&amp;mode=info&amp;id={$otherRequestRow.ally_id}">{$otherRequestRow.ally_name}</a>{else}-
						{/if}
					</td>
					<td><a
							href="game.php?page=galaxy&amp;galaxy={$otherRequestRow.galaxy}&amp;system={$otherRequestRow.system}">[{$otherRequestRow.galaxy}:{$otherRequestRow.system}:{$otherRequestRow.planet}]</a>
					</td>
					<td>{$otherRequestRow.text}</td>
					<td><a href="game.php?page=buddyList&amp;mode=accept&amp;id={$otherRequestID}"><img
								src="styles/resource/images/true.png" alt="{$LNG.bu_accept}" title="{$LNG.bu_accept}"></a><a
							href="game.php?page=buddyList&amp;mode=delete&amp;id={$otherRequestID}"><img
								src="styles/resource/images/false.png" alt="{$LNG.bu_decline}" title="{$LNG.bu_decline}"></a></td>
				</tr>
			{foreachelse}
				<tr>
		...
		{if !empty($myRequestList)}
			...
			{foreach $myRequestList as $myRequestID => $myRequestRow}
				<tr>
					...
					<td><a
							href="game.php?page=galaxy&amp;galaxy={$myRequestRow.galaxy}&amp;system={$myRequestRow.system}">[{$myRequestRow.galaxy}:{$myRequestRow.system}:{$myRequestRow.planet}]</a>
					</td>
					<td>{$myRequestRow.text}</td>
					<td>
						<a href="game.php?page=buddyList&amp;mode=delete&amp;id={$myRequestID}"><img
								src="styles/resource/images/false.png" alt="{$LNG.bu_cancel_request}"
								title="{$LNG.bu_cancel_request}">
						</a>
					</td>
				</tr>
			{foreachelse}
		...
		{foreach $myBuddyList as $myBuddyID => $myBuddyRow}
			<tr>
				...
				<td><a href="game.php?page=buddyList&amp;mode=delete&amp;id={$myBuddyID}"><img
							src="styles/resource/images/false.png" alt="{$LNG.bu_delete}" title="{$LNG.bu_delete}"></a></td>
			</tr>
		{foreachelse}
			<tr>
