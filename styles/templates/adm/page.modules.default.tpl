...
<div class="bg-black w-75  p-3 my-3 mx-auto fs-12">
  <div class="form-group d-flex justify-content-between">
    ...
    <input style="max-width:250px;" id="searchInModules" class="form-control py-1 bg-dark  my-1 border border-secondary" type="text" name="" placeholder="search..">
  </div>
  ...
  <div id="modules" class="form-group d-flex justify-content-center row my-1 border border-1 border-secondary">
  	...
  	{if $Info.state == 1}
  		...
  		<a class="col fs-5 " href="?page=module&mode=change&type=deaktivate&id={$ID}">{$LNG.mod_change_deactive}</a>
  	{else}
  		<span class="col fs-5" style="color:red">{$LNG.mod_deactive}</span>
      <a class="col fs-5 " href="?page=module&mode=change&type=activate&id={$ID}">{$LNG.mod_change_active}</a>
  	{/if}
  </div>
  {/foreach}
</div>
