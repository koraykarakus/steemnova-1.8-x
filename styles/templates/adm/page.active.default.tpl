{block name="content"}
	...
	<script type="text/javascript">
		function activeUser(validationID, validationKey) {
			$.getJSON('index.php?page=verify&mode=json&i=' + validationID + '&k=' + validationKey, function(data) {
				alert(data);
	...
	<table class="table table-dark table-striped table-sm fs-12 w-50 my-5 mx-auto">
		...
		{foreach name=User item=User from=$Users}
			<tr>
				...
				<td>
					<a href="?page=active&amp;mode=delete&id={$User.id}"
						onclick="return confirm('{$LNG.ap_sicher}{$User.username} {$LNG.ap_entfernen}');">
