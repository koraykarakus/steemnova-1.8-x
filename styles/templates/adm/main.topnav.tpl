<div style="height:50px;" class="d-flex align-items-center justify-content-center text-white fw-bold">
...
<div class="d-flex justify-content-center align-items-center">
	...
	<a href="admin.php?page=overview" target="Hauptframe"
		class="border mx-1 p-1 border-white rounded text-decoration-none text-white fs-6">{$LNG.adm_cp_index}</a>
	{if $authlevel == $smarty.const.AUTH_ADM}
		<a href="?page=universe&amp;sid={$sid}" target="Hauptframe"
			class="border mx-1 p-1 border-white rounded text-decoration-none text-white fs-6">{$LNG.mu_universe}</a>
		<a href="?page=rights" target="Hauptframe"
			class="border mx-1 p-1 border-white rounded text-decoration-none text-white fs-6">{$LNG.mu_moderation_page}</a>
		<a href="?page=rights&amp;mode=users&amp;sid={$sid}" target="Hauptframe"
			class="border mx-1 p-1 border-white rounded text-decoration-none text-white fs-6">{$LNG.ad_authlevel_title}</a>
	{/if}
	{if $id == 1}
		<a href="?page=reset&amp;sid={$sid}" target="Hauptframe"
			class="border mx-1 p-1 border-white rounded text-decoration-none text-white fs-6">{$LNG.re_reset_universe}</a>
	{/if}
