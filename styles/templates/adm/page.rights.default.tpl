{block name="content"}
	<form action="" method="post" name="users">
		<table class="table table-dark table-striped table-sm fs-12 w-50 my-5 mx-auto">
			...
			<tr>
				<td colspan="2">
					<select class="w-100" name="id_1" size="20">
						{foreach $users as $c_user}
							<option value="{$c_user['id']}">
								{$c_user['username']}&nbsp;(
								{if $c_user['authlevel'] == 0}
									{$LNG.rank_0}
								{elseif $c_user['authlevel'] == 1}
									{$LNG.rank_1}
								{elseif $c_user['authlevel'] == 2}
									{$LNG.rank_2}
								{elseif $c_user['authlevel'] == 3}
									{$LNG.rank_3}
								{/if}
								)
							</option>
					...
					<INPUT TYPE="button" onClick="UserList.reset();this.form.regexp.value=''"
						value="{$LNG.button_deselect}">
				</td>
			</tr>
			<tr>
				<td class="d-flex justify-content-center" colspan="3">
